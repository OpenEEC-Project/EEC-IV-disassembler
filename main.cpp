...
int main (int argc, const char **argv)
{
  ...
  char* pth[3];
  prt_stars();
  printf ("EEC-IV disassembler Version 3.02     ");
       ...
       switch (*x)
        {
         ...
         case 'H':
           ...
           printf ("\nsad  <-c configdir> <-w configdir> <filename>");
           printf ("\n   options");
           ...
           printf ("\n      asks for filename if none specified\n");
          return 0;
...
/*******************************/
 ...
 // prt_stars();
    ...
    if (fn)
       {
        printf ("\nFor binary file '%s'\n", fn);
        go = dissas(fn);                                //(char *) argv[1]);      // use parameter and no loop
